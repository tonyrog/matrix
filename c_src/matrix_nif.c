		   ...
		   size_t n, size_t m)
{
	...
	float64_t (*unaryf)(float64_t) = unaryop_float64[func];
	...
	complex128_t (*unaryf)(complex128_t) = unaryop_complex128[func];
	...
	int64_t (*unaryf)(int64_t) = unaryop_int64[func];
	for (i=0; i<n; i++) {
	    byte_t* ap1 = ap;
	    byte_t* cp1 = cp;
	    for (j = 0; j < m; j++) {
		   ...
		   size_t n, size_t m)
{
	...
	void (*write_cf)(byte_t*, float64_t) = write_float64_func[ct];
	float64_t (*binop)(float64_t, float64_t) = binop_float64[func];
	...
	void (*write_cf)(byte_t*, complex128_t) = write_complex128_func[ct];
	complex128_t (*binop)(complex128_t,complex128_t)=binop_complex128[func];
	...
	void    (*write_cf)(byte_t*, int64_t) = write_int64_func[ct];
	int64_t (*binop)(float64_t, int64_t) = binop_int64[func];
		   ...
		   size_t n, size_t m, int32_t* cp)
{
	...
	while(m--) {
	    ...
	    int32_t i = 1;
	    int32_t max_i = 1;
	    float64_t max_v = read_float64(at, ap1);

	    ap1 += au;
	    while(n1--) {
		float64_t v = read_float64(at, ap1);
		ap1 += au;
		i++;
	...
	while(m--) {
	    ...
	    int32_t i = 1;
	    int32_t max_i = 1;
	    complex128_t max_v = read_complex128(at, ap1);

	    ap1 += au;
	    while(n1--) {
		complex128_t v = read_complex128(at, ap1);
		ap1 += au;
		i++;
	...
	while(m--) {
	    ...
	    int32_t i = 1;
	    int32_t max_i = 1;
	    int64_t max_v = read_int64(at, ap1);

	    ap1 += au;
	    while(n1--) {
		int64_t v = read_int64(at, ap1);
		ap1 += au;
		i++;
		if (v > max_v) { max_v = v; max_i = i; }
	    }
		  ...
		  size_t n, size_t m)
{
    ...
    if (is_float(ct)) {
	float64_t max_v = read_float64(at, ap);
	while(m--) {
	    byte_t* ap1 = ap;
	    size_t  n1 = n;
	    if (cv) {
		max_v = read_float64(at, ap1);
		ap1 += au;
		n1--;
	    }
	    while(n1--) {
	    ...
	    write_float64(ct, cp, max_v);
	    cp += cv;
	    ap += av;
	}
    }
    else if (is_complex(ct)) {
	complex128_t max_v = read_complex128(at, ap);
	while(m--) {
	    byte_t* ap1 = ap;
	    size_t  n1 = n;
	    if (cv) {
		max_v = read_complex128(at, ap1);
		ap1 += au;
		n1--;
	    }
	    while(n1--) {
		complex128_t v = read_complex128(at, ap1);
		ap1 += au;
		if (cabs(v) > cabs(max_v)) { max_v = v; }
	    }
	    write_complex128(ct, cp, max_v);
	    cp += cv;
	    ap += av;
	}
    }
    else {
	int64_t max_v = read_int64(at, ap);
	while(m--) {
	    byte_t* ap1 = ap;
	    size_t  n1 = n;
	    if (cv) {
		max_v = read_int64(at, ap1);
		ap1 += au;
		n1--;
	    }
	    while(n1--) {
		  ...
		  size_t n, size_t m)
{
    ...
    if (is_float(ct)) {
	float64_t min_v = read_float64(at, ap);
	while(m--) {
	    byte_t* ap1 = ap;
	    size_t  n1 = n;
	    if (cv) {
		min_v = read_float64(at, ap1);
		ap1 += au;
		n1--;
	    }
	    while(n1--) {
	    ...
	    write_float64(ct, cp, min_v);
	    cp += cv;
	    ap += av;
	}
    }
    else if (is_complex(ct)) {
	complex128_t min_v = read_complex128(at, ap);
	while(m--) {
	    byte_t* ap1 = ap;
	    size_t  n1 = n;
	    if (cv) {
		min_v = read_complex128(at, ap1);
		ap1 += au;
		n1--;
	    }
	    while(n1--) {
		complex128_t v = read_complex128(at, ap1);
		ap1 += au;
		if (cabs(v) < cabs(min_v)) { min_v = v; }
	    }
	    write_complex128(ct, cp, min_v);
	    cp += cv;
	    ap += av;
	}
    }
    else {
	int64_t min_v = read_int64(at, ap);
	while(m--) {
	    byte_t* ap1 = ap;
	    size_t  n1 = n-1;
	    if (cv) {
		min_v = read_int64(at, ap1);
		ap1 += au;
		n1--;
	    }	    
	    while(n1--) {
...
#endif
	{
	    mt_multiply(at,ap,au,av,an,am,bp,bu,bv,bn,bm,kp,ku,kv,cp,cu,cv);
	}
    }
    else {
	byte_t* bp0 = bp;
...
static matrix_t* init_matrix(matrix_t* mp,
			     ...
			     bool_t rowmajor, unsigned int stride,
				...
				bool_t rowmajor,matrix_type_t type,size_t align)
{
	...
	if ((mp->base = enif_alloc(size+align-1)) != NULL) {
	    mp->size   = size;
	    mp->stride = byte_stride / element_size(type);
...
static int get_matrix(ErlNifEnv* env, ERL_NIF_TERM arg, matrix_t* mp)
{
    ...
    byte_stride = mp->stride*element_size(type);

    if (ptr && enif_get_resource(env, elems[8], matrix_r, (void**)&rmp) &&
	(rmp->ptr == ptr)) {
	if (rmp->stride != mp->stride)
	    return 0;
	// check bounds, we may have a submatrix
	if ((byte_offset + (mp->n-1)*byte_stride +
	...
	ErlNifBinary bin;
	if (!enif_inspect_binary(env, elems[8], &bin))
	    return 0;
	// check bounds, we may have a submatrix
	if ((byte_offset + (mp->n-1)*byte_stride +
	     mp->m*element_size(type)) > bin.size)
	    return 0;
	mp->size = bin.size;
	mp->base = NULL;
	mp->data = bin.data;
...
// get_scalar_matrix
// get a scalar and represent it as a nxm matrix
// stride=0 signal scalar (repetetive) data! for now.
// if the base type is the same as the erlang scalar base type
// i.e base type = integer,float or comlex
//
			     ...
			     unsigned int n, unsigned int m)
{
	...
	return 0;
    init_matrix(mp, n, m, rowmajor, 0, type, mp->sdata.data);
    return 1;
}
...
ERL_NIF_TERM matrix_create(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
	...
	if (c.stride == c.m)
	    ...
	    size_t  au = c.stride*element_size(type);
	    size_t i;
	    for (i=0; i<n; i++) {
		memcpy(ap, bp, bu);
		ap += au;
			    ...
			    const ERL_NIF_TERM argv[])
{
	...
	if ((j < 1) || (j > a.m))
	    return enif_make_badarg(env);
	ptr = a.first + (i-1)*size_of_array(a.type, a.stride) +
	...
	if ((i < 1) || (i > a.m))
	    return enif_make_badarg(env);
	ptr = a.first + (j-1)*size_of_array(a.type, a.stride) +
...
{
    int av = ap->stride ? 1 : 0;
    bool_t use_vector = av;
    
    if (cp->rowmajor == ap->rowmajor)
	func(use_vector,
	     ap->type, ap->first, ap->stride, av,
	     cp->type, cp->first, cp->stride, 1, cp->n, cp->m);
    else
	func(FALSE,
	     ap->type, ap->first, av, ap->stride,
	     cp->type, cp->first, cp->stride, 1, cp->n, cp->m);
}
...
{
    int av = ap->stride ? 1 : 0;
    int bv = bp->stride ? 1 : 0;
    bool_t use_vector = av && bv;

    if (cp->rowmajor) {
	if (ap->rowmajor && bp->rowmajor)
	    func(use_vector,
		ap->type, ap->first, ap->stride, av,
		bp->type, bp->first, bp->stride, bv,
    ...
    matrix_t* ap, matrix_t* bp, matrix_type_t ct, byte_t* cp)
{
    if (ap->rowmajor && bp->rowmajor)
	func(TRUE,
	     ap->type, ap->first, ap->stride, 1,
	     bp->type, bp->first, bp->stride, 1,
	     ct, cp, ap->n, ap->m);
    else if (!ap->rowmajor && bp->rowmajor)
	func(FALSE,
	     ap->type, ap->first, 1, ap->stride,
	     bp->type, bp->first, bp->stride, 1,
	     ct, cp, bp->n, bp->m);
    else if (ap->rowmajor && !bp->rowmajor)
	func(FALSE,
	     ap->type, ap->first, ap->stride, 1,
	     bp->type, bp->first, 1, bp->stride,
	     ct, cp, ap->n, ap->m);
    else
	func(TRUE,
	     ap->type, ap->first, 1, ap->stride,
...
ERL_NIF_TERM matrix_topk(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    if (a.rowmajor)
	topk(k, a.type, a.first, a.stride, c.first, 1, m);
    else
...
ERL_NIF_TERM matrix_l2pool(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
	...
	if (!create_matrix(env,cn,cm,a.rowmajor,a.type,&c,&bin))
	    return enif_make_badarg(env);
	matrix_r_lock(&a);
	l2pool(a.type, a.first, a.stride, 1, a.n, a.m,
	       c.type, c.first, c.stride, 1, c.n, c.m,
	       ru, rv, rn, rm);
	matrix_r_unlock(&a);
	return make_matrix(env, c.n, c.m, c.rowmajor, c.type, &c, bin);
    }
    else { // argc == 6 with destination matrix
	if (!get_matrix(env, argv[5], &c))
	    return enif_make_badarg(env);
	...
	if (a.rowmajor == c.rowmajor) {
	    ...
	    matrix_rw_lock(&a,&c);
	    l2pool(a.type, a.first, a.stride, 1, a.n, a.m,
		   c.type, c.first, 1, c.stride, c.m, c.n,
		   ru, rv, rn, rm);
	    matrix_rw_unlock(&a,&c);
	}
	return argv[5];
...
ERL_NIF_TERM matrix_maxpool(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
	...
	if (!create_matrix(env,cn,cm,a.rowmajor,a.type,&c,&bin))
	    return enif_make_badarg(env);
	matrix_r_lock(&a);
	maxpool(a.type, a.first, a.stride, 1, a.n, a.m,
		c.type, c.first, c.stride, 1, c.n, c.m,
		ru, rv, rn, rm);
	matrix_r_unlock(&a);
	return make_matrix(env, c.n, c.m, c.rowmajor, c.type, &c, bin);
...
ERL_NIF_TERM matrix_filter(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
	...
	wn = w.n; wm = w.m; wu = w.stride; wv = 1;
    }
    else {
	wn = w.m; wm = w.n; wu = 1, wv = w.stride;
    }
	...
	if (!create_matrix(env,cn,cm,a.rowmajor,a.type,&c,&bin))
	    return enif_make_badarg(env);
	matrix_r_lock(&a);
	filter(a.type, a.first, a.stride, 1, a.n, a.m,
	...
	if (a.rowmajor == c.rowmajor) {
	    ...
	    matrix_rw_lock(&a,&c);
	    filter(a.type, a.first, 1, a.stride, a.m, a.n,
...
ERL_NIF_TERM matrix_copy(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
	...
	matrix_rw_lock(&a, &c);
	if (a.rowmajor == c.rowmajor)
	    tile(a.type, a.first, a.stride, 1, a.n, a.m,
		 c.type, c.first, c.stride, 1, c.n, c.m,
		 repeat_m, repeat_n);
	else
	    tile(a.type, a.first, 1, a.stride, a.n, a.m,
		 c.type, c.first, c.stride, 1, c.n, c.m,
		 repeat_m, repeat_n);
...
ERL_NIF_TERM matrix_fill(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    matrix_rw_lock(&a, &c);
    if (a.rowmajor == c.rowmajor)
	fill(a.type, a.first, a.stride, 1, a.n, a.m,
		  c.type, c.first, c.stride, 1, c.n, c.m);
    else
	fill(a.type, a.first, 1, a.stride, a.n, a.m,
	     c.type, c.first, c.stride, 1, c.n, c.m);
    matrix_rw_unlock(&a, &c);
...
ERL_NIF_TERM matrix_sigmoid(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    matrix_r_lock(&a);
    sigmoid(a.type, a.first, a.stride, 1,
				  ...
				  const ERL_NIF_TERM argv[])
{
    ...
    matrix_r_lock(&y);
    sigmoid_prime1(y.type, y.first, y.stride, 1,
		  c.type, c.first, c.stride, 1,
		  c.n, c.m);
    matrix_r_unlock(&y);
    return make_matrix(env,c.n,c.m,c.rowmajor,c.type,&c,bin);
			 ...
			 const ERL_NIF_TERM argv[])
{
    ...
    matrix_r_lock(&a);
    rectifier(a.type, a.first, a.stride, 1,
	      c.type, c.first, c.stride, 1,
	      c.n, c.m);
    matrix_r_unlock(&a);
    return make_matrix(env,c.n,c.m,c.rowmajor,c.type,&c,bin);
			     ...
			     const ERL_NIF_TERM argv[])
{
    ...
    col_step = elem_size;
    row_step = size_of_array(type, c.stride);
			   ...
			   const ERL_NIF_TERM argv[])
{
	...
	if (axis == 0) {
		...
		return enif_make_badarg(env);
	    matrix_r_lock(&a);
	    argmax(a.type, a.first,a.stride,1,a.n,a.m,
		   (int32_t*)c.first);
	    matrix_r_unlock(&a);
	    return make_matrix(env,c.n,c.m,c.rowmajor,c.type,&c,bin);
	}
		...
		return enif_make_badarg(env);
	    matrix_r_lock(&a);
	    argmax(a.type, a.first,1,a.stride,a.m,a.n,
		   (int32_t*)c.first);
	    matrix_r_unlock(&a);
	    return make_matrix(env,c.n,c.m,c.rowmajor,c.type,&c,bin);
	}
